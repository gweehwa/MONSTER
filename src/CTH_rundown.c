            ...
            double xtrain_to_est_ratio, double propensity)
{
	...
	Rprintf("Entered CTH_rundown. tmp_obs and temp_id are %d and %d.\n", tmp_obs, tmp_id);   
		...
		n++;
  	Rprintf("Entered CTH_rundown. ct.IV, ct.ydata, ct.treatment are %f, %f and %f.\n", ct.IV[tmp_obs], *ct.ydata[tmp_obs], ct.treatment[tmp_obs]);      
    ...
    Rprintf("Entered CTH_rundown. det is %f.\n", det);
    if (det != 0){
    det = 1.0 / det;
