...
// PARAMETER!                    
    ...
    */
        ...
        for (i = 0; i < nclass; i++) {
            if (countn[i] > 0) {
                tsplit[i] = RIGHT;
        alpha_1 = (countn[j] * xz_sum - x_sum * z_sum) / (countn[j] * xy_sum - x_sum * y_sum);
        alpha_0 = (z_sum - alpha_1 * y_sum) / wtsums[j];
        beta_1 = (countn[j] * xy_sum - x_sum * y_sum) / (countn[j] * xx_sum - x_sum * x_sum);
        beta_0 = (y_sum - beta_1 * x_sum) / countn[j];
        temp = alpha_1;
