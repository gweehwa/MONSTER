    ...
    if (nclass == 0) {
        ...
        for (i = 0; right_n > edge; i++) {
            ...
            if (x[i + 1] != x[i] && left_n >= edge &&
                ...
                Rprintf("Entered CT.c. Left treatment effect, variace and effect are %.2f, %.2f, %.2f", left_temp, (numerator/denominator), left_effect);
// PARAMETER!                    
                ...
                right_effect = alpha * right_temp * right_temp * right_wt - (1 - alpha) * (1 + train_to_est_ratio) 
                    * right_wt * (numerator / denominator);
                Rprintf("Entered CT.c. Right treatment effect, variace and effect are %.2f, %.2f, %.2f", right_temp, (numerator/denominator), right_effect);
