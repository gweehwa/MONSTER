        ...
        int bucketnum, int bucketMax, double *train_to_est_ratio)
{
    if (who == 1 && maxcat > 0) {
        ...
        x2x1_sumc = x1x4_sumc + maxcat;
        x2x2_sumc = x2x1_sumc + maxcat;
        x2x3_sumc = x2x2_sumc + maxcat;
        x2x4_sumc = x2x3_sumc + maxcat;
        x3x1_sumc = x2x4_sumc + maxcat;
        x3x2_sumc = x3x1_sumc + maxcat;
        x3x3_sumc = x3x2_sumc + maxcat;
        x3x4_sumc = x3x3_sumc + maxcat;
        x4x1_sumc = x3x4_sumc + maxcat;
        x4x2_sumc = x4x1_sumc + maxcat;
        x4x3_sumc = x4x2_sumc + maxcat;
        x4x4_sumc = x4x3_sumc + maxcat;
...
//    }
    ...
    *risk = 4 * twt * max_y * max_y - twt * (alpha * bhat_1*bhat_1 + (1 - alpha) * bhat_2*bhat_2) +  (1 + train_to_est_ratio) * twt * (alpha * var1 + (1 - alpha) * var2);
// PARAMETER! 
...
//    }
    ...
    ////node_effect = alpha * (bhat_1 * bhat_1 + bhat_2 * bhat_2)/2 * right_wt - (1 - alpha) * (1 + train_to_est_ratio)  * right_wt * (var1 + var2)/2;
    node_effect = (alpha * bhat_1 * bhat_1 + (1 - alpha) * bhat_2 * bhat_2) * right_wt - (1 + train_to_est_ratio)  * right_wt * (alpha * var1 + (1 - alpha) * var2);
...
//    det = 1.0 / det; //may need if(det = 0)
    ...
    Rprintf("Denominator is zero.\n");
                ...
                left_effect = (alpha * bhat_1 * bhat_1 + (1 - alpha) * bhat_2 * bhat_2) * left_wt - (1 + train_to_est_ratio) * left_wt * (alpha * var1 + (1 - alpha) * var2);
...
//Rprintf("Entered CT.c. right_x1x2_sum, right_x1x3_sum, right_x1x4_sum, right_x2x2_sum, right_x2x3_sum, right_x2x4_sum,   right_x3x3_sum, right_x3x4_sum right_x4x4_sum are %.2f, %.2f, %.2f, %.2f., %.2f., %.2f., %.2f., %.2f., %.2f.\n",  right_x1x2_sum, right_x1x3_sum, right_x1x4_sum, right_x2x2_sum, right_x2x3_sum, right_x2x4_sum, right_x3x3_sum, right_x3x4_sum, right_x4x4_sum );
                ...
                right_effect = (alpha * bhat_1 * bhat_1 + (1 - alpha) * bhat_2 * bhat_2) * right_wt - (1 + train_to_est_ratio) * right_wt * (alpha * var1 + (1 - alpha) * var2);
